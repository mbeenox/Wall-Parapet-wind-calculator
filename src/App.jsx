...
function Diagram({ H: h, pw: pwv, hL: hl, qL: qlv, hR: hr, qR: qrv, fmt }) {
  ...
  return (
    <svg viewBox="0 0 740 510" style={{ width: "100%", height: "100%", display: "block", minHeight: 420 }}>
      ...
      {/* Wind arrows — Right parapet (leeward suction, pointing left to right away from wall) */}
      {hr > 0 && qrv > 0 && <WindArrows x1={rx + pW + 4} x2={rx + pW + 60} yTop={rTop + 6} yBot={wallTop - 6}
        count={Math.max(2, Math.floor(hr / 2))} color={C.violet} label={`${fmt(qrv)} psf`} />}
...
function WindArrows({ x1, x2, yTop, yBot, count, color, label }) {
  ...
  return (
    <g>
      {Array.from({ length: n }, (_, i) => {
        const y = yTop + dy * i;
        return <line key={i} x1={x1} y1={y} x2={x2} y2={y} stroke={color} strokeWidth={1.8}
